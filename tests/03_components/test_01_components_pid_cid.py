...
async def get_component(client, get_projects):
    ...
    if a_component is None:
        response = await client.post(f'/projects/{project_id}/components', json={
...
async def test_create_level_0_component_by_project_id(client, get_projects):
    ...
    assert response_json == {'description': 'Level 0 component',
                             'id': 2,
                             'level': 0,
                             'parentId': None,
                             'projectId': project_id,
                             'sequence': 2,
                             'slug': 'level-0-component',
                             'title': 'Level 0 component',
                             }
    # project_a - comp(id:1, lvl:0)
    #           + comp(id:2, lvl:0)
...
@pytest.mark.asyncio
async def test_create_level_0_component_by_project_id_with_duplicate_title(client, get_projects):
    # project_a - comp(id:1, lvl:0)
    #           + comp(id:2, lvl:0)
    # project_b
    project_id = get_projects['project_a']['id']
    ...
    assert response.json() == {
        'detail': [{'input': 'Level 0 component',
                    'loc': ['body', 'title'],
                   ...
                   {'input': 'level-0-component',
                    'loc': ['body', 'slug'],
                    'msg': "A level 0 component with slug 'level-0-component' "
                    'already exists for project',
                    'type': 'value_error'}]}


@pytest.mark.asyncio
async def test_create_level_0_component_with_conflicting_project_id_in_body(client, get_projects):
    # project_a - comp(id:1, lvl:0)
    #           + comp(id:2, lvl:0)
    # project_b
    ...
    response = await client.post(f'/projects/{project_id_one}/components', json={
...
@ pytest.mark.asyncio
async def test_create_level_0_component_with_nonexistant_project_id_in_path(client, get_projects):
    # project_a - comp(id:1, lvl:0)
    #           + comp(id:2, lvl:0)
    # project_b
    response = await client.post(f'/projects/99/components', json={
        "title": "Level 0 component",
        "description": "Level 0 component",
        "level": 0
    })
...
@ pytest.mark.asyncio
async def test_create_level_0_component_with_missing_project_id_in_path(client, get_projects):
    # project_a - comp(id:1, lvl:0)
    #           + comp(id:2, lvl:0)
    # project_b
    project_id = get_projects['project_a']['id']
    response = await client.post(f'/projects//components', json={
        "title": "Level 0 component",
        "description": "Level 0 component",
        "level": 0
    })
...
@ pytest.mark.asyncio
async def test_create_level_0_component_with_neg_project_id_in_path(client, get_projects):
    # project_a - comp(id:1, lvl:0)
    #           + comp(id:2, lvl:0)
    # project_b
    response = await client.post(f'/projects/-2/components', json={
        "title": "Level 0 component",
        "description": "Level 0 component",
        "level": 0
    })
...
@ pytest.mark.asyncio
async def test_create_level_0_component_with_parent_id_in_the_path(client, get_projects, get_component):
    # project_a - comp(id:1, lvl:0)
    #           + comp(id:2, lvl:0)
    # project_b
    project_id = get_projects['project_a']['id']
    component_id = get_component["id"]
    response = await client.post(f'/projects/{project_id}/components/{component_id}', json={
        "title": "Level 0 component",
        "description": "Level 0 component",
        "level": 0
    })
    assert response.status_code == 400
    assert response.json() == {
        'detail': 'A level 0 component cannot have a parent ID'}


@ pytest.mark.asyncio
async def test_create_level_0_component_with_parent_id_in_the_body(client, get_projects, get_component):
    # project_a - comp(id:1, lvl:0)
    #           + comp(id:2, lvl:0)
    # project_b
    project_id = get_projects['project_a']['id']
    ...
    assert response.json() == {
        'detail': [{'ctx': {'error': {}},
                    'input': {'description': 'Level 0 component',
                              'level': 0,
                              'parentId': 1,
                              'title': 'Level 0 component'},
                    'loc': ['body'],
...
async def test_create_component_without_title(client, get_projects):
    ...
    assert response.json() == {'detail': [{'type': 'missing', 'loc': [
        'body', 'title'], 'msg': 'Field required', 'input': {'description': 'Component without a title', 'level': 0}}]}
...
async def test_create_component_with_empty_title(client, get_projects):
    ...
    assert response.status_code == 422
    assert response.json() == {'detail': [{'ctx': {'min_length': 3},
                                           'input': '',
                                           'loc': ['body', 'title'],
...
async def test_create_component_with_long_title(client, get_projects):
    ...
    assert response.status_code == 422
    assert response.json() == {'detail': [{'ctx': {'max_length': 100},
...
@ pytest.mark.asyncio
async def test_create_level_1_component_with_parent_id_in_the_path_and_body_mistmatch(client, get_projects, get_component):
    # project_a - comp(id:1, lvl:0)
    #           + comp(id:2, lvl:0)
    # project_b
    project_id = get_projects['project_a']['id']
...
@ pytest.mark.asyncio
async def test_create_level_1_component_with_parent_id(client, get_projects, get_component):
    # project_a - comp(id:1, lvl:0)
    #           + comp(id:2, lvl:0)
    # project_b
    project_id = get_projects['project_a']['id']
...
@ pytest.mark.asyncio
async def test_create_second_level_1_component_with_parent_id(client, get_projects, get_component):
    # project_a - comp(id:1, lvl:0) - comp(id:3, lvl:1)
    #                               + comp(id:4, lvl:1)
    #           + comp(id:2, lvl:0)
    # project_b
...
@ pytest.mark.asyncio
async def test_create_level_1_component_with_duplicate_title_for_parent_id(client, get_projects, get_component):
    # project_a - comp(id:1, lvl:0) - comp(id:3, lvl:1)
    #                               + comp(id:4, lvl:1)
    #           + comp(id:2, lvl:0)
    # project_b
    ...
    assert response.status_code == 422
    assert response.json() == {
        'detail': [{'input': 'first-level-1-component',
                    'loc': ['body', 'slug'],
                    'msg': "A component with slug 'first-level-1-component' "
                    'already exists for parent ID 1',
                    'type': 'value_error'}]}


@ pytest.mark.asyncio
async def test_create_level_1_component_with_duplicate_slug_for_parent_id(client, get_projects, get_component):
    # project_a - comp(id:1, lvl:0) - comp(id:3, lvl:1)
    #                               + comp(id:4, lvl:1)
    #           + comp(id:2, lvl:0)
    # project_b
...
@pytest.mark.asyncio
async def test_get_level_0_component_with_descendants(client, get_projects, get_component):
    # project_a - comp(id:1, lvl:0) - comp(id:3, lvl:1)
    #                               + comp(id:4, lvl:1)
    #           + comp(id:2, lvl:0)
    # project_b
    project_id = get_projects['project_a']['id']
    component_id = get_component["id"]
    response = await client.get(f'/projects/{project_id}/components/{component_id}/with-descendants')
    assert response.status_code == 200

    response_json = remove_uuid(response.json())
    for descendant in response_json["descendants"]:
        del descendant["uuid"]
    ...
    assert response_json == {
        ...
        'projectId': project_id,
        'sequence': 1,
        'slug': 'level-0-component-from-fixture',
        'title': 'Level 0 component from fixture'}
...
@ pytest.mark.asyncio
async def test_create_level_3_component_with_level_0_parent_id(client, get_projects, get_component):
    # project_a - comp(id:1, lvl:0) - comp(id:3, lvl:1)
    #                               + comp(id:4, lvl:1)
    #           + comp(id:2, lvl:0)
    # project_b
    ...
    assert response.status_code == 422
    assert response.json() == {'detail': [{'input': 1, 'loc': [
        'body', 'parent_id'], 'msg': 'Component level can only be one higher than the parent level', 'type': 'value_error'}]}


@pytest.mark.asyncio
async def test_create_level_0_component_with_duplicate_sequence_no(client, get_projects):
    # project_a - comp(id:1, lvl:0) - comp(id:3, lvl:1)
    #                               + comp(id:4, lvl:1)
    #           + comp(id:2, lvl:0)
    # project_b
...
@ pytest.mark.asyncio
async def test_create_level_2_component_without_a_title(client, get_projects, get_component):
    # project_a - comp(id:1, lvl:0) - comp(id:3, lvl:1)
    #                               + comp(id:4, lvl:1)
    #           + comp(id:2, lvl:0)
    # project_b
    ...
    assert response.status_code == 422
    assert response.json() == {'detail': [{'type': 'missing', 'loc': [
        'body', 'title'], 'msg': 'Field required', 'input': {'description': 'Level 2 component', 'level': 2}}]}
...
async def test_update_component_with_new_title(client, get_projects, get_component):
    ...
    response = await client.put(f'/projects/{project_id}/components/{component["id"]}', json={
        "title": "Updated title"
    })
    assert response.status_code == 200
...
async def test_update_level_0_component_with_duplicate_title(client, get_projects):
    ...
    response = await client.put(f'/projects/{project_id}/components/{project_one["id"]}', json={
...
async def test_update_component_with_new_description(client, get_projects):
    ...
    response = await client.put(f'/projects/{project_id}/components/3', json={
        "description": "This in a longish updated description"
    })
    assert response.status_code == 200
    assert response.json() == predicted_component
...
async def test_update_component_with_new_parent_id(client, get_projects):
    ...
    response = await client.get(f'/projects/{project_id}/components/{component_one["id"]}/descendants')
    ...
    response = await client.put(f'/projects/{project_id}/components/{component_one_first_descendant["id"]}', json={
        "parentId": component_two["id"]})
    assert response.status_code == 200
    assert response.json() == predicted_component


@ pytest.mark.asyncio
async def test_update_component_with_new_parent_id_but_duplicate_sequence(client, get_projects):
    ...
    response = await client.post(f'/projects/{project_id}/components/4', json={
    ...
    project_id = get_projects['project_a']['id']
    response = await client.post(f'/projects/{project_id}/components/3', json={
    ...
    response = await client.put(f'/projects/{project_id}/components/{component_1_4_1["id"]}', json={
        "parentId": 3
    })
    assert response.status_code == 422
    assert response.json() == {
...
@ pytest.mark.asyncio
async def test_update_component_with_none_existant_parent_id_in_the_body(client, get_projects):
    # project_a - comp(id:1, lvl:0) - comp(id:4, lvl:1) - comp(id:5, lvl:2, seq:1)
    #           + comp(id:2, lvl:0) - comp(id:3, lvl:1) - comp(id:6, lvl:2, seq:1)
    # project_b
    ...
    response = await client.put(f'/projects/{project_id}/components/{lvl_1_component["id"]}',
...
async def test_delete_component_with_descendants(client, get_projects, get_component):
    ...
    assert response.status_code == 422
    assert response.json() == {'detail': [{'type': 'value_error', 'loc': [
        'body', 'id'], 'msg': 'Cannot delete a component with descendants', 'input': 1}]}


@ pytest.mark.asyncio
async def test_delete_component_without_descendants(client, get_projects, get_component):
    # project_a - comp(id:1, lvl:0) - comp(id:4, lvl:1) - comp(id:5, lvl:2, seq:1)
    #           + comp(id:2, lvl:0) - comp(id:3, lvl:1) - comp(id:6, lvl:2, seq:1)
    # project_b
