...
from app.pydantic_models.role_model import (
    Role,
    RoleCreate,
    RoleUpdate,
)
...
async def create_role(
    ...
    try:
        role_in_db = await SqlRole.create(
            db, user_id=current_user.id, **role.model_dump()
        )
...
async def update_role(
    ...
    try:
        role_in_db = await SqlRole.update(
            db, user_id=current_user.id, id=id, **role.model_dump()
        )
    except ValueError as error:
        raise HTTPException(status_code=400, detail=str(error))
    return role_in_db
...
async def delete_role(
    ...
    try:
        result = await SqlRole.delete(db, id)
    except ValueError as error:
        raise HTTPException(status_code=400, detail=str(error))
    return result
...
# Replace with add_role_to_user and remove_role_from_user
#
