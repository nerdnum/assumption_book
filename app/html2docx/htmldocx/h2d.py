...
class HtmlToDocx(HTMLParser):
    ...
    def add_styles_to_paragraph(self, style):
        ...
        if "docx-style" in style:
            ## Nerdnum added this to handle document titles on 2025-02-21
            ## The font-style should match the styles available in the document
            ## otherwise it will throw an error
            paragraph_style = self.doc.styles[style["docx-style"]]
            self.apply_paragraph_style(paragraph_style)
        ...
        if image:
            try:
                if isinstance(self.doc, docx.document.Document):
                    self.doc.add_picture(image, width=Inches(6.25))
                else:
    ...
    def parse_html_string(self, html, template=None):
        if template:
            template_docx = Document(template)
            self.set_initial_attrs(document=template_docx)
        else:
            self.set_initial_attrs()
        self.run_process(html)
        return self.doc
